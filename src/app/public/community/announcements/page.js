...
class AnnouncementsPage extends App {
    ...
    async loadPageData() {
        ...
        if (globalState.announcementsPageData) {
            this.set('pageData', globalState.announcementsPageData);
            this.render();
            return;
        }
    ...
    async fetchPageData() {
        try {
            const response = await api.get('/pages/slug/announcements');
            if (response.data.success) {
                const pageData = response.data.data;
                
                // Cache the data in global store
                store.setState({ announcementsPageData: pageData });
                        ...
                        <div class="relative">
                            ...
                            <div class="relative w-full h-96">
                                <img src="${this.getImageUrl(this.getBannerImages(pageData)[0])}" 
                                     alt="Announcements Banner Image" 
                                        ...
                                        <div class="relative group">
                                            <div class="relative w-full h-32">
                                                <img src="${this.getImageUrl(imagePath)}" 
                                                     alt="Announcements Gallery Image ${index + 2}" 
