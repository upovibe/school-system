 ...
 * It renders within the global RootLayout and fetches data for the "news" slug.
...
class NewsPage extends App {
    ...
    async loadPageData() {
        ...
        if (globalState.newsPageData) {
            this.set('pageData', globalState.newsPageData);
            this.render();
            return;
        }
    ...
    async fetchPageData() {
        try {
            const response = await api.get('/pages/slug/news');
            if (response.data.success) {
                const pageData = response.data.data;
                
                // Cache the data in global store
                store.setState({ newsPageData: pageData });
    ...
    async refreshData() {
        // Clear the cache
        store.setState({ newsPageData: null });
                                        ...
                                        <div class="relative group">
                                            <div class="relative w-full h-32">
                                                <img src="${this.getImageUrl(imagePath)}" 
                                                     alt="News Gallery Image ${index + 2}" 
