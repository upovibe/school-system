 ...
 * It renders within the global RootLayout and fetches data for the "gallery" slug.
...
class GalleryPage extends App {
    ...
    async loadPageData() {
        ...
        if (globalState.galleryPageData) {
            this.set('pageData', globalState.galleryPageData);
            this.render();
            return;
        }
    ...
    async fetchPageData() {
        try {
            const response = await api.get('/pages/slug/gallery');
            if (response.data.success) {
                const pageData = response.data.data;
                
                // Cache the data in global store
                store.setState({ galleryPageData: pageData });
    ...
    async refreshData() {
        // Clear the cache
        store.setState({ galleryPageData: null });
