...
/**
 * Announcement Add Modal Component
 * 
 * A modal component for adding new announcements in the admin panel
 * 
 * Attributes:
 ...
 */
class AnnouncementAddModal extends HTMLElement {
    constructor() {
        super();
    }
    ...
    async saveAnnouncement() {
        try {
            ...
            // Create announcement
            const response = await api.withToken(token).post('/announcements', announcementData);
            ...
            if (response.status === 201 || response.data.success) {
                ...
                try {
                    const announcementResponse = await api.withToken(token).get(`/announcements/${response.data.data.id}`);
...
customElements.define('announcement-add-modal', AnnouncementAddModal);
