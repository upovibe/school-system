 ...
 * 
 * Handles CSV file import for students with validation and error reporting
 */
class StudentImportDialog extends HTMLElement {
    constructor() {
        super();
        this.file = null;
            ...
            if (this.file) {
                fileDisplay.innerHTML = `
                    <div class="flex items-center gap-3 p-3 bg-green-50 border border-green-200 rounded-lg">
                        ...
                        <button onclick="this.closest('student-import-dialog').clearFile()" 
                            ...
                            <div class="space-y-2 max-h-40 overflow-y-auto">
                                ${data.errors.map(error => `
                                    <div class="bg-red-50 border border-red-200 rounded p-2 text-sm">
                                        <div class="font-medium">Row ${error.row} (${error.student_id})</div>
    ...
    render() {
        this.innerHTML = `
            <ui-dialog 
                ...
                <div slot="footer">
                    <button onclick="this.closest('student-import-dialog').close()" 
                            class="px-3 py-1 text-gray-600 hover:text-gray-800 transition-colors">
                        Cancel
                    </button>
                    <button onclick="this.closest('student-import-dialog').handleImport()" 
    ...
    async handleImport() {
        ...
        try {
            ...
            Toast.show({
                title: 'Import Error',
                message: error.message || 'Failed to import students data',
                variant: 'error',
                duration: 5000
            });
...
customElements.define('student-import-dialog', StudentImportDialog);
