...
class SystemUpdateModal extends App {
    constructor() {
        super();
        this.state = {
            id: null,
    ...
    async handleUpdate(e) {
        ...
        try {
            const response = await api.withToken(token).put(`/settings/${this.state.id}`, this.state);
            if (response.data.success) {
                Toast.show({ title: 'Success', message: 'Setting updated successfully.', variant: 'success' });
                this.dispatchEvent(new CustomEvent('setting-updated', { bubbles: true, composed: true }));
                this.close();
            } else {
    ...
    render() {
        return `
            <ui-modal title="Update Setting">
                <form class="space-y-4" @submit="${this.handleUpdate}">
