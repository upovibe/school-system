...
class TeacherStudentGradeUpdateModal extends HTMLElement {
  constructor() { super(); this.gradeData = null; this.policy = null; }
  ...
  setGradeData(grade) {
    ...
    this.populate();
    if (grade && grade.subject_id) { this.loadPolicyForSubject(grade.subject_id); }
  }
  ...
  async update() {
    try {
      ...
      const payload = { assignment_total: assignVal, exam_total: examVal, remarks: (remarksTa?.getValue() || '') };
      const token = localStorage.getItem('token');
      ...
      const resp = await api.withToken(token).put(`/teacher/student-grades/${this.gradeData.id}`, payload);
      if (resp.data.success) {
        let full = null;
        try { const showResp = await api.withToken(token).get(`/student-grades/${this.gradeData.id}`); full = showResp?.data?.data || null; } catch(_) {}
        ...
        this.dispatchEvent(new CustomEvent('student-grade-updated', { detail: { grade: enriched }, bubbles: true, composed: true }));
        this.close();
      } else {
...
customElements.define('teacher-student-grade-update-modal', TeacherStudentGradeUpdateModal);
export default TeacherStudentGradeUpdateModal;
